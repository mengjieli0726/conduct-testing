...
#sc <- sparkR.session(appName = "wordcount_gpu.R")
sc <- sparkR.init(appName="SparkRwordcount")
...
lines1 <- read.text(sqlContext,args[[1]])
#tokens <- selectExpr(lines, "explode(split(value, ' ')) as word")
lines2 <- read.text(sqlContext,args[[1]])
...
cpu_tokens <- SparkR:::toRDD(lines1)
gpu_tokens <- SparkR:::toRDD(lines2)
